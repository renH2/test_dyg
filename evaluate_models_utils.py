...
def evaluate_model_node_classification(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler,
                                       ...
                                       time_gap: int = 2000, dataset_name: str = ""):
    """
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            if (dataset_name in ['BitAlpha', 'BitOtc']):
            ...
            if model_name in ['TGAT', 'CAWN', 'TCL', 'FastTGAT']:
                # get temporal embedding of source and destination nodes
                # two Tensors, with shape (batch_size, node_feat_dim)
                batch_src_node_embeddings, batch_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
...
def evaluate_model_node_classification_withtarget(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler,
                                                  evaluate_idx_data_loader: DataLoader,
                                                  evaluate_data: Data, loss_func: nn.Module, num_neighbors: int = 20,
                                                  time_gap: int = 2000, target_idx=[]):
    """
    evaluate models on the node classification task
    :param model_name: str, name of the model
    :param model: nn.Module, the model to be evaluated
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            for i in range(len(batch_src_node_ids)):
                if batch_src_node_ids[i] in target_idx:
                    indices.append(i)
            batch_target_idx = torch.from_numpy(np.array(indices)).int().to(predicts.device)

            loss = loss_func(input=predicts[batch_target_idx], target=labels[batch_target_idx])

            evaluate_total_loss += loss.item()
            evaluate_y_trues.append(labels[batch_target_idx])
            evaluate_y_predicts.append(predicts[batch_target_idx])
            evaluate_idx_data_loader_tqdm.set_description(
                f'evaluate for the {batch_idx + 1}-th batch, evaluate loss: {loss.item()}')

        evaluate_total_loss /= (batch_idx + 1)
        # evaluate_y_trues = torch.cat(evaluate_y_trues, dim=0)
        evaluate_y_trues = torch.cat([t.flatten() for t in evaluate_y_trues])
        # evaluate_y_predicts = torch.cat(evaluate_y_predicts, dim=0)
        evaluate_y_predicts = torch.cat([t.flatten() for t in evaluate_y_predicts])
        evaluate_metrics = get_node_classification_metrics(predicts=evaluate_y_predicts, labels=evaluate_y_trues)
