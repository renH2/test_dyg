...
class TGAT(nn.Module):
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        if self.neighbor_sampler.edge_sampler is not None:
            self.edge_sampler = self.neighbor_sampler.edge_sampler  # 关键修改点
            self.edge_sampler = self.edge_sampler.to(device)
        else:
            self.edge_sampler = self.neighbor_sampler.edge_sampler
    ...
    def compute_src_dst_node_temporal_embeddings_with_mutual(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        src_node_embeddings, src_mutual_loss = self.compute_node_temporal_embeddings_with_mutual(node_ids=src_node_ids,
                                                                                                 node_interact_times=node_interact_times,
                                                                                                 current_layer_num=self.num_layers,
                                                                                                 num_neighbors=num_neighbors)
        # Tensor, shape (batch_size, node_feat_dim)
        dst_node_embeddings, dst_mutual_loss = self.compute_node_temporal_embeddings_with_mutual(node_ids=dst_node_ids,
                                                                                                 node_interact_times=node_interact_times,
                                                                                                 current_layer_num=self.num_layers,
                                                                                                 num_neighbors=num_neighbors)
        return src_node_embeddings, dst_node_embeddings, src_mutual_loss + dst_mutual_loss
    ...
    def compute_mutual(self, matrix_a, matrix_b):
        batch_size = matrix_a.shape[0]
        sim_matrix = torch.matmul(matrix_a, matrix_b.transpose(0, 1)) / self.temperature
        labels = torch.arange(batch_size).to(matrix_a.device)

        loss = F.cross_entropy(sim_matrix, labels)
        return loss

    def compute_node_temporal_embeddings_with_mutual(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        if current_layer_num == 0:
            ...
            if self.edge_sampler is not None:
                output1, _ = self.temporal_conv_layers[current_layer_num - 1](node_features=node_conv_features,
                ...
                center_node = node_conv_features.unsqueeze(1).expand(-1, neighbor_node_conv_features.shape[1], -1)
                fused_feat = torch.cat([center_node, neighbor_node_conv_features, neighbor_time_features], dim=-1)
                neighbor_probs = self.edge_sampler(fused_feat)
                neighbor_node_conv_features = neighbor_node_conv_features * neighbor_probs.unsqueeze(-1)  # [B, N, D]
            ...
            output, _ = self.temporal_conv_layers[current_layer_num - 1](node_features=node_conv_features,
                                                                         ...
                                                                         neighbor_node_edge_features=neighbor_edge_features,
                                                                         neighbor_masks=neighbor_node_ids)
            loss_mutual = self.compute_mutual(output, output1)
