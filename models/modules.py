...
class FastMultiAttention(nn.Module):
    def __init__(self, node_feat_dim, time_feat_dim, edge_feat_dim, num_heads, dropout):
        super().__init__()
        self.num_heads = num_heads
        self.head_dim = (node_feat_dim + time_feat_dim) // num_heads
        self.scale = self.head_dim ** -0.5
        total_dim = node_feat_dim + time_feat_dim + edge_feat_dim
        self.qkv_proj = nn.Linear(total_dim, 3 * num_heads * self.head_dim, bias=False)
        self.residual_fc = nn.Linear(node_feat_dim + time_feat_dim, num_heads * self.head_dim)
        self.layer_norm = nn.LayerNorm(node_feat_dim + time_feat_dim)
        self.dropout = nn.Dropout(dropout)

    def forward(self, node_features, node_time_features, neighbor_node_features, neighbor_node_edge_features, neighbor_node_time_features, neighbor_masks):
        # 拼接节点自身特征
        node_feat = torch.cat([node_features.unsqueeze(1), node_time_features.unsqueeze(1)], dim=2)
        # 拼接邻居特征
        neighbor_feat = torch.cat([neighbor_node_features, neighbor_node_edge_features, neighbor_node_time_features],
                                  dim=2)

        B, N, _ = neighbor_feat.shape
        # 合并全量输入（把 self 与 neighbor 一起做投影）
        all_feat = torch.cat([node_feat, neighbor_feat], dim=1)  # shape (B, 1+N, D)
        qkv = self.qkv_proj(all_feat)  # (B, 1+N, 3*H*hd)
        qkv = qkv.view(B, -1, 3, self.num_heads, self.head_dim)  # (B, 1+N, 3, H, hd)
        qkv = qkv.permute(2, 0, 3, 1, 4)  # (3, B, H, 1+N, hd)
        query, key, value = qkv[0], qkv[1], qkv[2]  # 各 (B, H, 1+N, hd)

        # 只取第一条作为 node->neigh attention
        query = query[:, :, :1, :]  # (B, H, 1, hd)
        key = key[:, :, 1:, :]  # (B, H, N, hd)
        value = value[:, :, 1:, :]  # (B, H, N, hd)

        mask = neighbor_masks[:, None, None, :] == 0

        attn_scores = torch.matmul(query, key.transpose(-2, -1)) * self.scale
        attn_scores = attn_scores.masked_fill(mask, -1e10)  # **[加速点4]** 一次 mask 操作
        attn_probs = F.softmax(attn_scores, dim=-1)
        attn_probs = self.dropout(attn_probs)
        attention_output = torch.matmul(attn_probs, value)  # (B, H, 1, hd)

        # 合并 heads
        attention_output = attention_output.transpose(2, 1).reshape(B, 1, self.num_heads * self.head_dim)
        # 残差+前馈
        residual = torch.cat([node_features.unsqueeze(1), node_time_features.unsqueeze(1)], dim=2)
        out = self.residual_fc(attention_output)
        out = self.dropout(out)
        out = self.layer_norm(out + residual)
        out = out.squeeze(1)  # (B, D_out)

        # 返回注意力权重 & 输出
        attn_probs = attn_probs.squeeze(2)  # (B, H, N)
