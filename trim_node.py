...
from utils.utils import set_random_seed, convert_to_gpu, get_parameter_sizes, create_optimizer, create_optimizer_new
...
from utils.DataLoader import get_idx_data_loader, get_node_classification_data_withtarget_ratio, \
    get_node_classification_dgraph, calculate_temporal_pagerank
from utils.EarlyStopping import EarlyStopping
from utils.load_configs import get_node_classification_args, test_args, get_method_args
import subprocess


def get_gpu_memory_by_pid(pid=None):
    """获取当前程序在 GPU 上的显存占用（单位：MB）"""
    if pid is None:
        pid = os.getpid()  # 获取当前进程的 PID

    try:
        # 执行 nvidia-smi 命令，筛选特定 PID
        ...
        for line in result.strip().split("\n"):
            ...
            if int(p) == pid:
                return float(mem)
        return 0.0  # 未找到对应 PID
    except Exception as e:
        return f"Error: {e}"


def get_ram_memory_by_pid(pid=None):
    """获取当前程序在 RAM 上的内存占用（单位：MB）"""
    if pid is None:
        pid = os.getpid()  # 获取当前进程的 PID
    try:
...
if __name__ == "__main__":
    ...
    #     get_n ode_classification_data_withtarget_ratio(dataset_name=args.dataset_name, val_ratio=args.val_ratio,
    ...
    if args.sample_neighbor_strategy == 'all_sample':
        args.num_neighbors = full_neighbor_sampler.max_neighbor_number

    number_ram_list = []
    number_gpu_memory_list = []
    time_cost_list = []
    sampling_time_list = []

    print("Current number neighbors", args.num_neighbors)
    ...
    for run in range(args.num_runs):
        ...
        args.save_model_name = f'node_classification_{args.model_name}_seed{args.seed}_withtarget_{str(args.target_ratio)}_{str(args.freeze)}'
        ...
        logger.info(
            f'model name: {args.model_name}, #parameters: {get_parameter_sizes(model) * 4} B, 'f'{get_parameter_sizes(model) * 4 / 1024} KB, {get_parameter_sizes(model) * 4 / 1024 / 1024} MB.')
        optimizer = create_optimizer_new(model=model, optimizer_name=args.optimizer, learning_rate=args.learning_rate,
                                         weight_decay=args.weight_decay, idx=args.idx)
        ...
        if args.random is True:
            save_model_folder = root + f"/saved_models_random/{args.model_name}/{args.dataset_name}/{args.save_model_name}"
        else:
            save_model_folder = root + f"/saved_models/{args.model_name}/{args.dataset_name}/{args.save_model_name}/"
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.model_name in ['TGAT', 'CAWN', 'TCL']:
                    if args.model_name == 'TGAT' and args.mutual == 1:
                        batch_src_node_embeddings, batch_dst_node_embeddings, loss_mutual = \
                            model[0].compute_src_dst_node_temporal_embeddings_with_mutual(
                ...
                loss.backward()
                optimizer.step()
                if args.freeze is False and args.model_name in ['JODIE', 'DyRep', 'TGN']:
                    model[0].memory_bank.detach_memory_bank()
            # print("Sampling time", sampling_time)
            t4 = time.time()
            # print("Training time", training_time)
            training_time = training_time + t4 - t3
            ...
            logger.info(f"Sampling time this epoch: {sampling_time}")
            sampling_time_list.append(sampling_time)
            if args.gputest:
                gpu_used = get_gpu_memory_by_pid()
                number_gpu_memory_list.append(gpu_used)
                logger.info(f'the used GPU memory is {gpu_used}')
                ram_used = get_ram_memory_by_pid()
                number_ram_list.append(ram_used)
                logger.info(f'the used RAM is {ram_used}')

            train_total_loss_list.append(train_total_loss)
            train_roc_list.append(train_metrics["roc_auc"])
            logger.info(
        ...
        test_total_loss, test_metrics = evaluate_model_node_classification(model_name=args.model_name, model=model,
                                                                           neighbor_sampler=full_neighbor_sampler,
                                                                           evaluate_idx_data_loader=test_idx_data_loader,
                                                                           evaluate_data=test_data,
                                                                           loss_func=loss_func,
        ...
        print("#########")
        print("AUC", float(json.loads(result_json)["test metrics"]["roc_auc"]))
